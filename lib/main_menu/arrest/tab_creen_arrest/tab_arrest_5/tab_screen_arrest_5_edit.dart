...
class _TabScreenArrest5CreateState extends State<TabScreenArrest5Edit> {
  ...
  void initState() {
    ...
    widget.ItemsData.forEach((item) {
      ...
      editProductDesc.text = item.REMARK;
    ...
    for (int i = 0; i < widget.ItemsData.length; i++) {
      ...
      itemsController[i].editQuantityUnit.text = widget.ItemsData[i].QUANTITY_UNIT != null ? widget.ItemsData[i].QUANTITY_UNIT.toString() : "";
  ...
  Widget _buildBottom() {
    ...
    return Container(
      ...
      child: MaterialButton(
        onPressed: () {
          for (int i = 0; i < widget.ItemsData.length; i++) {
            /*if (itemsController[i].editQuantity.text.isEmpty ||
                itemsController[i].editVolumeUnit.text.isEmpty) {
              new VerifyDialog(context, 'กรุณากรอกจำนวนและปริมาณสุทธิ');
            } else {*/
